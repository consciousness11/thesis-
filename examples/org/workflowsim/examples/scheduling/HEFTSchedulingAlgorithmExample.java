/**
 * HEFT planner (with STATIC scheduler so the scheduler does not override the
 * planner's assignment) over the same four scientific workflows used by the
 * DQN and PPO examples -- Montage, CyberShake, Epigenomics, LIGO -- at the
 * 100-task setting. Reports makespan and an estimated energy consumption per
 * workflow using the same formula as the other examples so the three are
 * directly comparable.
 */
...
public class HEFTSchedulingAlgorithmExample {
    ...
    public static void main(String[] args) {
        ...
        Log.enable();
        Log.printLine();
        Log.printLine("===== HEFT summary (100 tasks per workflow) =====");
    ...
    private static double[] runOne(String name, String daxPath) {
        ...
        try {
            Parameters.SchedulingAlgorithm sch = Parameters.SchedulingAlgorithm.STATIC;
            Parameters.PlanningAlgorithm pln = Parameters.PlanningAlgorithm.HEFT;
